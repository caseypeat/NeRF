...
class NerfRenderer(nn.Module):
    ...
    def render(self, rays_o, rays_d, bound, bg_color):
        ...
        z_vals_log = torch.cat([torch.linspace(m.log10(self.inner_near), m.log10(self.inner_far), self.inner_steps, device=rays_o.device), torch.linspace(m.log10(self.outer_near), m.log10(self.outer_far), self.outer_steps, device=rays_o.device)], dim=-1)[None, :].expand(rays_o[0].shape[0], -1)
        ...
        image, invdepth, weights = helpers.render_rays_log_new(sigmas, rgbs, z_vals_log)
