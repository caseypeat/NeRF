...
from sdf.utils import rend_util
...
class ErrorBoundSampler(RaySampler):
    ...
    def get_z_vals(self, ray_dirs, cam_loc, model):
        beta0 = model.laplace_density.get_beta().detach()
        ...
        while not_converge and total_iters < self.max_total_iters:
            ...
            with torch.no_grad():
                ...
                samples_sdf = model.get_sdf_vals(points_flat)  # remove implicit network
            ...
            d_star = torch.zeros(z_vals.shape[0], z_vals.shape[1] - 1, device='cuda')
            # print(d_star.dtype, b.dtype, z_vals.dtype)
            # exit()
            ...
            density = model.laplace_density(sdf.reshape(z_vals.shape), beta=beta.unsqueeze(-1))
    ...
    def get_error_bound(self, beta, model, sdf, z_vals, dists, d_star):
        density = model.laplace_density(sdf.reshape(z_vals.shape), beta=beta)
